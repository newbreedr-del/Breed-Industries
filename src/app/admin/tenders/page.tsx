...
import {
  Search, Filter, RefreshCw, Plus, Users, FileText,
...
export default function TendersDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [tRes, mRes] = await Promise.all([
        ...
        fetch('/api/tender-clients'), // reuse for stats count
      ]);
      if (tRes.ok) {
      ...
      if (allMatchRes.ok) {
        ...
        setStats(s => ({ ...s, activeClients: cData.total ?? 0 }));
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const loadMatches = useCallback(async () => {
    try {
      // Fetch matches per tender (for the matches tab)
      ...
      setMatches(allMatches.sort((a, b) => (b.match_score - a.match_score)));
      setStats(s => ({
        ...s,
        openTenders: ts.length,
        newMatches:  allMatches.filter(m => m.status === 'new').length,
  ...
  const runScrape = async () => {
    ...
    try {
      ...
      setScrapeResult(
        res.ok
          ? `✅ Scraped ${d.scraped} tenders · ${d.newTenders} saved · ${d.matches} matches · ${d.notified} emails sent`
          : `❌ ${d.error}`
      );
      await load();
      await loadMatches();
    ...
    <>
      ...
      >
        <div className="flex gap-3">
          <Link href="/admin/tender-clients" className="btn btn-outline flex items-center gap-2">
      ...
      <section className="py-16 bg-color-bg-secondary relative">
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          {/* Scrape result */}
          {scrapeResult && (
            <div className={`p-4 rounded-lg mb-6 flex items-start justify-between gap-3 ${
              scrapeResult.startsWith('✅') ? 'bg-green-500/10 border border-green-500/30' : 'bg-red-500/10 border border-red-500/30'
            }`}>
              <p className="text-sm text-white/80">{scrapeResult}</p>
              <button onClick={() => setScrapeResult(null)}><X size={16} className="text-white/50" /></button>
            </div>
          )}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/10 text-white/50 text-xs uppercase tracking-wider">
                      ...
                      <th className="px-5 py-3 text-left">Title / Department</th>
                  ...
                  <tbody>
                    {filtered.map((t, i) => {
                      ...
                      return (
                        <tr key={t.id} className={`border-b border-white/5 hover:bg-white/3 transition-colors ${i % 2 === 0 ? '' : 'bg-white/2'}`}>
                          <td className="px-5 py-3 font-mono text-xs text-accent">{t.reference_number}</td>
                          ...
                          <td className="px-5 py-3 text-right text-white/70">{fmtRand(t.estimated_value)}</td>
                          <td className="px-5 py-3">
                  ...
                  <tbody>
                    {filteredMatches.map((m, i) => (
                      <tr key={m.id} className={`border-b border-white/5 hover:bg-white/3 transition-colors ${i % 2 === 0 ? '' : 'bg-white/2'}`}>
                        <td className="px-5 py-3">
                          <p className="text-white font-medium">{m.client?.company_name}</p>
